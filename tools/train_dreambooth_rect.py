...
def parse_args(input_args=None):
    ...
    parser.add_argument("--width", type=int, default=None, help="training width.")
    parser.add_argument("--height", type=int, default=None, help="training height.")
    ...
    def __init__(
        ...
        if size_width!=size_height:
            self.image_transforms = transforms.Compose(
                [
                    # transforms.Resize((self.size_height,self.size_width), interpolation=transforms.InterpolationMode.BILINEAR),
                    transforms.ToTensor(),
                    transforms.Normalize([0.5], [0.5]),
                ]
            )
        elif size_width==size_height:
            self.image_transforms = transforms.Compose(
                [
                    transforms.Resize(self.size_height, interpolation=transforms.InterpolationMode.BILINEAR),
                    transforms.CenterCrop(self.size_height) if center_crop else transforms.RandomCrop(self.size_height),
                    transforms.ToTensor(),
                    transforms.Normalize([0.5], [0.5]),
                ]
            )
